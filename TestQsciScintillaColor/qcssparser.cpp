...
static const short indexOfId[NumKnownValues] = {
    0,  41, 48, 42, 49, 50, 55, 35, 26, 71, 72, 25, 43, 5,  64, 48, 29, 59, 60,
    27, 52, 62, 6,  10, 39, 56, 19, 13, 17, 18, 20, 21, 51, 24, 46, 68, 37, 3,
    2,  40, 63, 16, 11, 58, 14, 32, 65, 33, 66, 56, 67, 34, 70, 8,  28, 38, 12,
    36, 61, 7,  9,  4,  69, 54, 22, 23, 30, 31, 1,  15, 0,  53, 45, 44};
...
static const QCssKnownValue attachments[NumKnownAttachments - 1] = {
    {"fixed", Attachment_Fixed}, {"scroll", Attachment_Scroll}};
...
static bool operator<(const QString &name, const QCssKnownValue &prop) {
  return QString::compare(name, QLatin1String(prop.name), Qt::CaseInsensitive) <
         0;
}

static bool operator<(const QCssKnownValue &prop, const QString &name) {
  return QString::compare(QLatin1String(prop.name), name, Qt::CaseInsensitive) <
         0;
}

static quint64 findKnownValue(const QString &name, const QCssKnownValue *start,
                              int numValues) {
  const QCssKnownValue *end = &start[numValues - 1];
  const QCssKnownValue *prop = std::lower_bound(start, end, name);
  if ((prop == end) || (name < *prop)) return 0;
  return prop->id;
}
...
LengthData ValueExtractor::lengthValue(const Value &v) {
  ...
  if (data.unit != LengthData::None) s.chop(2);
...
int ValueExtractor::lengthValue(const Declaration &decl) {
  ...
  if (decl.d->values.count() < 1) return 0;
...
bool ValueExtractor::extractGeometry(int *w, int *h, int *minw, int *minh,
                                     int *maxw, int *maxh) {
  extractFont();
  bool hit = false;
...
bool ValueExtractor::extractPosition(int *left, int *top, int *right,
...
QSize ValueExtractor::sizeValue(const Declaration &decl) {
  ...
  if (decl.d->values.count() > 0) x[0] = lengthValue(decl.d->values.at(0));
  if (decl.d->values.count() > 1)
    x[1] = lengthValue(decl.d->values.at(1));
  else
    x[1] = x[0];
  QList<QVariant> v;
  v << QVariant::fromValue<LengthData>(x[0])
    << QVariant::fromValue<LengthData>(x[1]);
  decl.d->parsed = v;
...
void ValueExtractor::sizeValues(const Declaration &decl, QSize *radii) {
  ...
  for (int i = 1; i < 4; i++) radii[i] = radii[0];
}

bool ValueExtractor::extractBorder(int *borders, QBrush *colors,
                                   BorderStyle *styles, QSize *radii) {
  extractFont();
  bool hit = false;
  for (int i = 0; i < declarations.count(); i++) {
    const Declaration &decl = declarations.at(i);
    switch (decl.d->propertyId) {
      ...
      case BorderLeft:
        borderValue(decl, &borders[LeftEdge], &styles[LeftEdge],
                    &colors[LeftEdge]);
        break;
      case BorderTop:
        borderValue(decl, &borders[TopEdge], &styles[TopEdge],
                    &colors[TopEdge]);
        break;
      case BorderRight:
        borderValue(decl, &borders[RightEdge], &styles[RightEdge],
                    &colors[RightEdge]);
        break;
      case BorderBottom:
        borderValue(decl, &borders[BottomEdge], &styles[BottomEdge],
                    &colors[BottomEdge]);
        break;
      case Border:
        borderValue(decl, &borders[LeftEdge], &styles[LeftEdge],
                    &colors[LeftEdge]);
        borders[TopEdge] = borders[RightEdge] = borders[BottomEdge] =
            borders[LeftEdge];
        styles[TopEdge] = styles[RightEdge] = styles[BottomEdge] =
            styles[LeftEdge];
        colors[TopEdge] = colors[RightEdge] = colors[BottomEdge] =
            colors[LeftEdge];
        break;
...
bool ValueExtractor::extractOutline(int *borders, QBrush *colors,
                                    BorderStyle *styles, QSize *radii,
                                    int *offsets) {
  extractFont();
  bool hit = false;
...
static Qt::Alignment parseAlignment(const QCss::Value *values, int count) {
  ...
  for (int i = 0; i < qMin(2, count); i++) {
    if (values[i].type != Value::KnownIdentifier) break;
...
ColorData Parser::parseColorValue(QCss::Value v) {
  if (v.type == Value::Identifier || v.type == Value::String) {
    v.variant.convert(QMetaType::QColor);
    v.type = Value::Color;
  }

  if (v.type == Value::Color) return qvariant_cast<QColor>(v.variant);

  if (v.type == Value::KnownIdentifier &&
      v.variant.toInt() == Value_Transparent)
    return QColor(Qt::transparent);

  if (v.type != Value::Function) return ColorData();

  QStringList lst = v.variant.toStringList();
  if (lst.count() != 2) return ColorData();

  const QString &identifier = lst.at(0);
  if ((identifier.compare(QLatin1String("palette"), Qt::CaseInsensitive)) ==
      0) {
  ...
  if (!rgb && !hsv && !hsl) return ColorData();

  const bool hasAlpha =
      identifier.size() == 4 && identifier.at(3) == QLatin1Char('a');
  if (identifier.size() > 3 && !hasAlpha) return ColorData();

  Parser p(lst.at(1));
  if (!p.testExpr()) return ColorData();

  QVector<QCss::Value> colorDigits;
  if (!p.parseExpr(&colorDigits)) return ColorData();
  ...
  for (int i = 0; i < qMin(tokenCount, 7); i += 2) {
    if (colorDigits.at(i).type == Value::Percentage) {
      ...
      colorDigits[i].variant =
          colorDigits.at(i).variant.toReal() * (maxRange / 100.);
  ...
  if (tokenCount < 5) return ColorData();

  // ### Qt6: replace this with a check and return invalid color when token
  // count does not match
  if (hasAlpha && tokenCount != 7)
    qWarning(
        "QCssParser::parseColorValue: Specified color with alpha value but no "
        "alpha given: '%s'",
        qPrintable(lst.join(QLatin1Char(' '))));
  ...
  if (rgb) return QColor::fromRgb(v1, v2, v3, alpha);
  if (hsv) return QColor::fromHsv(v1, v2, v3, alpha);
...
BrushData Parser::parseBrushValue(const QCss::Value &v, const QPalette &pal) {
  ...
  if (v.type != Value::Function) return BrushData();

  QStringList lst = v.variant.toStringList();
  if (lst.count() != 2) return BrushData();

  QStringList gradFuncs;
  gradFuncs << QLatin1String("qlineargradient")
            << QLatin1String("qradialgradient")
            << QLatin1String("qconicalgradient") << QLatin1String("qgradient");
  int gradType = -1;
  ...
  int spread = -1;
  QStringList spreads;
  spreads << QLatin1String("pad") << QLatin1String("reflect")
  ...
  while (parser.hasNext()) {
    parser.skipSpace();
    if (!parser.test(IDENT)) return BrushData();
    QString attr = parser.lexem();
    parser.skipSpace();
    if (!parser.test(COLON)) return BrushData();
    parser.skipSpace();
    if (attr.compare(QLatin1String("stop"), Qt::CaseInsensitive) == 0) {
      ...
      if (cd.type == ColorData::Role) dependsOnThePalette = true;
      stops.append(
          QGradientStop(stop.variant.toReal(), colorFromData(cd, pal)));
    } else {
      parser.next();
      QCss::Value value;
  ...
  if (gradType == 0) {
    QLinearGradient lg(
        vars.value(QLatin1String("x1")), vars.value(QLatin1String("y1")),
        vars.value(QLatin1String("x2")), vars.value(QLatin1String("y2")));
    lg.setCoordinateMode(QGradient::ObjectBoundingMode);
    lg.setStops(stops);
    if (spread != -1) lg.setSpread(QGradient::Spread(spread));
    BrushData bd = QBrush(lg);
    if (dependsOnThePalette) bd.type = BrushData::DependsOnThePalette;
    return bd;
  }

  if (gradType == 1) {
    QRadialGradient rg(
        vars.value(QLatin1String("cx")), vars.value(QLatin1String("cy")),
        vars.value(QLatin1String("radius")), vars.value(QLatin1String("fx")),
        vars.value(QLatin1String("fy")));
    rg.setCoordinateMode(QGradient::ObjectBoundingMode);
    rg.setStops(stops);
    if (spread != -1) rg.setSpread(QGradient::Spread(spread));
  ...
  if (gradType == 2) {
    QConicalGradient cg(vars.value(QLatin1String("cx")),
                        vars.value(QLatin1String("cy")),
                        vars.value(QLatin1String("angle")));
    cg.setCoordinateMode(QGradient::ObjectBoundingMode);
    cg.setStops(stops);
    if (spread != -1) cg.setSpread(QGradient::Spread(spread));
...
void ValueExtractor::borderValue(const Declaration &decl, int *width,
                                 QCss::BorderStyle *style, QBrush *color) {
  if (decl.d->parsed.isValid()) {
    BorderData data = qvariant_cast<BorderData>(decl.d->parsed);
    *width = lengthValueFromData(data.width, f);
    *style = data.style;
    *color = data.color.type != BrushData::Invalid
                 ? brushFromData(data.color, pal)
                 : QBrush(QColor());
    return;
  }
  ...
  *color = QColor();
  ...
  if (decl.d->values.isEmpty()) return;
  ...
  int i = 0;
  if (decl.d->values.at(i).type == Value::Length ||
  ...
  data.color = Parser::parseBrushValue(decl.d->values.at(i), pal);
  ...
  for (int i = 0; i < values.count(); ++i) {
    ...
    if (v.type == Value::Uri) {
      *image = v.variant.toString();
      continue;
    } else if (v.type == Value::KnownIdentifier &&
    ...
    Repeat repeatAttempt = static_cast<Repeat>(
        findKnownValue(v.variant.toString(), repeats, NumKnownRepeats));
    ...
    *brush = Parser::parseBrushValue(v, pal);
  }
}

bool ValueExtractor::extractBackground(QBrush *brush, QString *image,
                                       Repeat *repeat, Qt::Alignment *alignment,
                                       Origin *origin, Attachment *attachment,
                                       Origin *clip) {
  bool hit = false;
  for (int i = 0; i < declarations.count(); ++i) {
    const Declaration &decl = declarations.at(i);
    ...
    switch (decl.d->propertyId) {
      ...
      case BackgroundImage:
        if (val.type == Value::Uri) *image = val.variant.toString();
        break;
      case BackgroundRepeat:
        if (decl.d->parsed.isValid()) {
          *repeat = static_cast<Repeat>(decl.d->parsed.toInt());
        } else {
          *repeat = static_cast<Repeat>(
              findKnownValue(val.variant.toString(), repeats, NumKnownRepeats));
      ...
      case Background:
        if (decl.d->parsed.isValid()) {
          ...
          *alignment = data.alignment;
          ...
          BrushData brushData;
          parseShorthandBackgroundProperty(decl.d->values, &brushData, image,
                                           repeat, alignment, pal);
...
static bool setFontSizeFromValue(QCss::Value value, QFont *font,
  ...
  if (value.type != Value::Length) return false;
...
static bool setFontStyleFromValue(const QCss::Value &value, QFont *font) {
  if (value.type != Value::KnownIdentifier) return false;
  switch (value.variant.toInt()) {
    case Value_Normal:
...
static bool setFontWeightFromValue(const QCss::Value &value, QFont *font) {
  ...
  if (value.type != Value::Number) return false;
...
static bool setFontFamilyFromValues(const QVector<QCss::Value> &values,
  ...
  for (int i = start; i < values.count(); ++i) {
    ...
    if (shouldAddSpace) family += QLatin1Char(' ');
    family += str;
    shouldAddSpace = true;
  }
...
static void setTextDecorationFromValues(const QVector<QCss::Value> &values,
                                        QFont *font) {
  for (int i = 0; i < values.count(); ++i) {
    if (values.at(i).type != Value::KnownIdentifier) continue;
...
static void parseShorthandFontProperty(const QVector<QCss::Value> &values,
                                       QFont *font, int *fontSizeAdjustment) {
  font->setStyle(QFont::StyleNormal);
  font->setWeight(QFont::Normal);
  ...
  for (int i = 0; i < declarations.count(); ++i) {
    ...
    switch (decl.d->propertyId) {
      ...
      case FontFamily:
        ...
        break;
      case FontKerning:
        setFontKerningFromValue(val, font);
        break;
      case TextDecoration:
...
bool ValueExtractor::extractPalette(QBrush *fg, QBrush *sfg, QBrush *sbg,
                                    QBrush *abg) {
  bool hit = false;
  for (int i = 0; i < declarations.count(); ++i) {
    const Declaration &decl = declarations.at(i);
...
QColor Declaration::colorValue(const QPalette &pal) const {
  if (d->values.count() != 1) return QColor();
  ...
  ColorData color = Parser::parseColorValue(d->values.at(0));
...
QBrush Declaration::brushValue(const QPalette &pal) const {
  if (d->values.count() != 1) return QBrush();
  ...
  BrushData data = Parser::parseBrushValue(d->values.at(0), pal);
...
void Declaration::brushValues(QBrush *c, const QPalette &pal) const {
  int needParse = 0x1f;  // bits 0..3 say if we should parse the corresponding
                         // value. the bit 4 say we need to update d->parsed
  int i = 0;
  if (d->parsed.isValid()) {
    needParse = 0;
  ...
  if (needParse != 0) {
    ...
    for (i = 0; i < qMin(d->values.count(), 4); i++) {
      if (!(needParse & (1 << i))) continue;
      BrushData data = Parser::parseBrushValue(d->values.at(i), pal);
      if (data.type == BrushData::Role) {
...
bool Declaration::realValue(qreal *real, const char *unit) const {
  if (d->values.count() != 1) return false;
  const Value &v = d->values.at(0);
  if (unit && v.type != Value::Length) return false;
  const QString str = v.variant.toString();
  QStringRef s(&str);
  if (unit) {
    const QLatin1String unitStr(unit);
    if (!s.endsWith(unitStr, Qt::CaseInsensitive)) return false;
    s.chop(unitStr.size());
  }
  bool ok = false;
...
QSize Declaration::sizeValue() const {
  if (d->parsed.isValid()) return qvariant_cast<QSize>(d->parsed);

  int x[2] = {0, 0};
  if (d->values.count() > 0) intValueHelper(d->values.at(0), &x[0], "px");
  if (d->values.count() > 1)
...
QRect Declaration::rectValue() const {
  if (d->values.count() != 1) return QRect();

  if (d->parsed.isValid()) return qvariant_cast<QRect>(d->parsed);

  const QCss::Value &v = d->values.at(0);
  if (v.type != Value::Function) return QRect();
  ...
  if (args.count() != 4) return QRect();
  QRect rect(args[0].toInt(), args[1].toInt(), args[2].toInt(),
             args[3].toInt());
...
void Declaration::colorValues(QColor *c, const QPalette &pal) const {
  ...
  if (d->parsed.isValid()) {
    ...
    QList<QVariant> v;
    for (i = 0; i < qMin(d->values.count(), 4); i++) {
      ColorData color = Parser::parseColorValue(d->values.at(i));
      if (color.type == ColorData::Role) {
...
BorderStyle Declaration::styleValue() const {
  if (d->values.count() != 1) return BorderStyle_None;
...
Repeat Declaration::repeatValue() const {
  if (d->parsed.isValid()) return static_cast<Repeat>(d->parsed.toInt());
  if (d->values.count() != 1) return Repeat_Unknown;
  int v = findKnownValue(d->values.at(0).variant.toString(), repeats,
                         NumKnownRepeats);
  d->parsed = v;
...
Origin Declaration::originValue() const {
  if (d->parsed.isValid()) return static_cast<Origin>(d->parsed.toInt());
  if (d->values.count() != 1) return Origin_Unknown;
  int v = findKnownValue(d->values.at(0).variant.toString(), origins,
                         NumKnownOrigins);
  d->parsed = v;
...
PositionMode Declaration::positionValue() const {
  if (d->parsed.isValid()) return static_cast<PositionMode>(d->parsed.toInt());
  if (d->values.count() != 1) return PositionMode_Unknown;
  int v = findKnownValue(d->values.at(0).variant.toString(), positions,
...
Attachment Declaration::attachmentValue() const {
  if (d->parsed.isValid()) return static_cast<Attachment>(d->parsed.toInt());
  if (d->values.count() != 1) return Attachment_Unknown;
  int v = findKnownValue(d->values.at(0).variant.toString(), attachments,
                         NumKnownAttachments);
  d->parsed = v;
...
int Declaration::styleFeaturesValue() const {
  ...
  if (d->parsed.isValid()) return d->parsed.toInt();
  int features = StyleFeature_None;
  for (int i = 0; i < d->values.count(); i++) {
    features |=
        static_cast<int>(findKnownValue(d->values.value(i).variant.toString(),
...
Qt::Alignment Declaration::alignmentValue() const {
  if (d->parsed.isValid()) return Qt::Alignment(d->parsed.toInt());
...
void Declaration::borderImageValue(QString *image, int *cuts, TileMode *h,
                                   TileMode *v) const {
  ...
  *h = *v = TileMode_Stretch;
  ...
  if (d->values.last().type == Value::Identifier) {
    *v = static_cast<TileMode>(findKnownValue(
        d->values.last().variant.toString(), tileModes, NumKnownTileModes));
  }
...
QIcon Declaration::iconValue() const {
  if (d->parsed.isValid()) return qvariant_cast<QIcon>(d->parsed);

  QIcon icon;
  for (int i = 0; i < d->values.count();) {
    const Value &value = d->values.at(i++);
    if (value.type != Value::Uri) break;
    ...
    if (i == d->values.count()) break;

    if (d->values.at(i).type == Value::TermOperatorComma) i++;
  }
...
int Selector::specificity() const {
  ...
  for (int i = 0; i < basicSelectors.count(); ++i) {
    ...
    if (!sel.elementName.isEmpty()) val += 1;
...
void StyleSheet::buildIndexes(Qt::CaseSensitivity nameCaseSensitivity) {
  ...
  for (int i = 0; i < styleRules.count(); ++i) {
    ...
    for (int j = 0; j < rule.selectors.count(); ++j) {
      ...
      if (selector.basicSelectors.isEmpty()) continue;

      if (selector.basicSelectors.at(0).relationToNext ==
          BasicSelector::NoRelation) {
        if (selector.basicSelectors.count() != 1) continue;
      } else if (selector.basicSelectors.count() <= 1) {
        continue;
      }

      const BasicSelector &sel =
          selector.basicSelectors.at(selector.basicSelectors.count() - 1);
...
bool StyleSelector::selectorMatches(const Selector &selector, NodePtr node) {
  ...
  if (selector.basicSelectors.at(0).relationToNext ==
      BasicSelector::NoRelation) {
    if (selector.basicSelectors.count() != 1) return false;
  ...
  do {
    ...
    if (!match) {
      if (i == selector.basicSelectors.count() -
                   1)  // first element must always match!
        break;
      if (sel.relationToNext != BasicSelector::MatchNextSelectorIfAncestor &&
          sel.relationToNext !=
              BasicSelector::MatchNextSelectorIfIndirectAdjecent)
        break;
    }
    ...
    if (sel.relationToNext == BasicSelector::MatchNextSelectorIfAncestor ||
      ...
      freeNode(node);
      node = nextParent;
    } else if (sel.relationToNext ==
                   BasicSelector::MatchNextSelectorIfDirectAdjecent ||
               sel.relationToNext ==
                   BasicSelector::MatchNextSelectorIfIndirectAdjecent) {
...
bool StyleSelector::basicSelectorMatches(const BasicSelector &sel,
                                         NodePtr node) {
    ...
    for (int i = 0; i < sel.attributeSelectors.count(); ++i) {
      ...
      switch (a.valueMatchCriterium) {
        ...
        case QCss::AttributeSelector::MatchBeginsWith:
          if (!attrValue.startsWith(a.value)) return false;
          break;
        case QCss::AttributeSelector::MatchEndsWith:
          if (!attrValue.endsWith(a.value)) return false;
          break;
        case QCss::AttributeSelector::MatchContains:
          if (!attrValue.contains(a.value)) return false;
          break;
      }
    }
  }
  ...
  if (!sel.ids.isEmpty() && sel.ids != nodeIds(node)) return false;

  return true;
}

void StyleSelector::matchRule(NodePtr node, const StyleRule &rule,
                              StyleSheetOrigin origin, int depth,
                              QMultiMap<uint, StyleRule> *weightedRules) {
  for (int j = 0; j < rule.selectors.count(); ++j) {
    const Selector &selector = rule.selectors.at(j);
    if (selectorMatches(selector, node)) {
      uint weight = rule.order + selector.specificity() * 0x100 +
      ...
      // We might have rules with the same weight if they came from a rule with
      // several selectors
...
QVector<StyleRule> StyleSelector::styleRulesForNode(NodePtr node) {
  ...
  if (styleSheets.isEmpty()) return rules;

  QMultiMap<uint, StyleRule>
      weightedRules;  // (spec, rule) that will be sorted below
  ...
  for (int sheetIdx = 0; sheetIdx < styleSheets.count(); ++sheetIdx) {
    ...
    for (int i = 0; i < styleSheet.styleRules.count(); ++i) {
      matchRule(node, styleSheet.styleRules.at(i), styleSheet.origin,
    ...
    if (!styleSheet.idIndex.isEmpty()) {
      ...
      for (int i = 0; i < ids.count(); i++) {
        ...
        QMultiHash<QString, StyleRule>::const_iterator it =
            styleSheet.idIndex.constFind(key);
        while (it != styleSheet.idIndex.constEnd() && it.key() == key) {
          matchRule(node, it.value(), styleSheet.origin, styleSheet.depth,
                    &weightedRules);
          ++it;
        }
      }
    ...
    if (!styleSheet.nameIndex.isEmpty()) {
      ...
      for (int i = 0; i < names.count(); i++) {
        ...
        if (nameCaseSensitivity == Qt::CaseInsensitive)
          name = std::move(name).toLower();
        QMultiHash<QString, StyleRule>::const_iterator it =
            styleSheet.nameIndex.constFind(name);
    ...
    if (!medium.isEmpty()) {
      for (int i = 0; i < styleSheet.mediaRules.count(); ++i) {
        if (styleSheet.mediaRules.at(i).media.contains(medium,
                                                       Qt::CaseInsensitive)) {
          for (int j = 0; j < styleSheet.mediaRules.at(i).styleRules.count();
               ++j) {
            matchRule(node, styleSheet.mediaRules.at(i).styleRules.at(j),
  ...
  for (; it != weightedRules.constEnd(); ++it) rules += *it;
...
QVector<Declaration> StyleSelector::declarationsForNode(
    NodePtr node, const char *extraPseudo) {
  ...
  for (int i = 0; i < rules.count(); i++) {
    ...
    if (pseudoClass == PseudoClass_Enabled ||
...
QString Scanner::preprocess(const QString &input, bool *hasEscapeSequences) {
  ...
  if (hasEscapeSequences) *hasEscapeSequences = false;
  ...
  while (i < output.size()) {
    if (output.at(i) == QLatin1Char('\\')) {
      ...
      if (hexCount == 0) {
        if (hasEscapeSequences) *hasEscapeSequences = true;
        continue;
      }
...
QString Symbol::lexem() const {
  QString result;
  if (len > 0) result.reserve(len);
  for (int i = 0; i < len; ++i) {
    if (text.at(start + i) == QLatin1Char('\\') && i < len - 1) ++i;
    result += text.at(start + i);
  }
  return result;
}

Parser::Parser(const QString &css, bool isFile) { init(css, isFile); }
...
bool Parser::parsePage(PageRule *pageRule) {
  ...
  do {
    ...
    if (!decl.isEmpty()) pageRule->declarations.append(decl);
...
bool Parser::parseProperty(Declaration *decl) {
  ...
  decl->d->propertyId = static_cast<Property>(
      findKnownValue(decl->d->property, properties, NumProperties));
...
bool Parser::parseRuleset(StyleRule *styleRule) {
  ...
  do {
    ...
    if (!decl.isEmpty()) styleRule->declarations.append(decl);
  } while (test(SEMICOLON));
...
bool Parser::parseNextDeclaration(Declaration *decl) {
  if (!testProperty()) return true;  // not an error!
...
bool Parser::parseExpr(QVector<Value> *values) {
  ...
  do {
    ...
    if (val.type != QCss::Value::Unknown) values->append(val);
    if (testTerm()) {
      onceMore = true;
      val = Value();
      if (!parseTerm(&val)) return false;
...
bool Parser::testTerm() {
  return test(PLUS) || test(MINUS) || test(NUMBER) || test(PERCENTAGE) ||
         test(LENGTH) || test(STRING) || test(IDENT) || testHexColor() ||
         testFunction();
}
...
bool Parser::parseFunction(QString *name, QString *args) {
  *name = lexem();
  ...
  for (int i = start; i < index - 1; ++i) args->append(symbols.at(i).lexem());
  /*
...
bool Parser::parseHexColor(QColor *col) {
  ...
  if (!col->isValid()) {
    qWarning("QCssParser::parseHexColor: Unknown color name '%s'",
...
bool Parser::testSimpleSelector() {
  return testElementName() || (test(HASH)) || testClass() || testAttrib() ||
         testPseudo();
}

bool Parser::next(QCss::TokenType t) {
  if (hasNext() && next() == t) return true;
  return recordError();
}

bool Parser::test(QCss::TokenType t) {
  if (index >= symbols.count()) return false;
...
QString Parser::lexemUntil(QCss::TokenType t) {
  QString lexem;
  while (hasNext() && next() != t) lexem += symbol().lexem();
  return lexem;
}
...
bool Parser::until(QCss::TokenType target, QCss::TokenType target2) {
  ...
  while (index < symbols.size()) {
    ...
    if ((t == target || (target2 != NONE && t == target2)) && braceCount <= 0 &&
        brackCount <= 0 && parenCount <= 0)
      return true;
