...
enum Edge { TopEdge, RightEdge, BottomEdge, LeftEdge, NumEdges };
...
struct ColorData {
  ...
  ColorData(const QColor &col)
      : color(col), role(QPalette::NoRole), type(Color) {}
...
// The Any specifier is never generated, but can be used as a wildcard in
// searches.
...
struct StyleSheet {
  ...
  Q_GUI_EXPORT void buildIndexes(
...
struct Q_GUI_EXPORT ValueExtractor {
  ...
  bool extractBackground(QBrush *, QString *, Repeat *, Qt::Alignment *,
                         QCss::Origin *, QCss::Attachment *, QCss::Origin *);
  bool extractGeometry(int *w, int *h, int *minw, int *minh, int *maxw,
                       int *maxh);
  bool extractPosition(int *l, int *t, int *r, int *b, QCss::Origin *,
                       Qt::Alignment *, QCss::PositionMode *, Qt::Alignment *);
  bool extractBox(int *margins, int *paddings, int *spacing = nullptr);
  bool extractBorder(int *borders, QBrush *colors, BorderStyle *Styles,
                     QSize *radii);
  bool extractOutline(int *borders, QBrush *colors, BorderStyle *Styles,
 ...
 private:
  void extractFont();
  void borderValue(const Declaration &decl, int *width,
